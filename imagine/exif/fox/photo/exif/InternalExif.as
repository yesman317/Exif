package nt.imagine.exif.fox.photo.exif
{
	import flash.utils.ByteArray;
	import flash.utils.Endian;
	
	import nt.imagine.exif.fox.photo.HexBinaryConverter;
	import nt.imagine.exif.fox.photo.exif.ifd.IFD;
	import nt.imagine.exif.fox.photo.exif.ifd.IFDGroup;
	...
	public class InternalExif
	{
		...
		public function read(tiff:ByteArray):void
		{
			...
			if (gpsIFD)
				this.gps = readIFDGroup(tiff, uint(gpsIFD.value),true);
				
			/*var gpsIFD:IFDValue = this.findTagById(37500);
			if (gpsIFD)
			{
				trace("gpsIFDgpsIFDgpsIFD "+gpsIFD.value +" "+gpsIFD.toString());
			}*/
		...
		private function readIFDGroup(tiff:ByteArray, start:uint,isGps:Boolean=false):IFDGroup
		{
			try
			{
				tiff.position=start;
